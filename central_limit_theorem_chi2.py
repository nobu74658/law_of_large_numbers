...
"""
中心極限定理の確認 - 左右非対称な分布（カイ二乗分布）を用いた実験
"""
...
def generate_sample_means(sample_size, num_experiments, df=1):
    """
...
def plot_sample_means_distribution(sample_sizes, num_experiments=10000, df=1):
    """
    ...
    # カイ二乗分布の理論的な平均と標準偏差
    theoretical_mean = df
    theoretical_std = np.sqrt(2 * df)  # カイ二乗分布の標準偏差は sqrt(2*df)
    ...
    for i, sample_size in enumerate(sample_sizes):
        ...
        # サンプル平均を生成
        sample_means = generate_sample_means(sample_size, num_experiments, df)
        ...
        x_min = max(0, theoretical_mean - 4 * std_of_mean)  # カイ二乗分布は正の値のみなので、0未満は除外
    ...
    # 全体のタイトルを追加
    plt.suptitle(f'Central Limit Theorem with Chi-Square Distribution (df={df})', 
                fontsize=16, y=1.02)
    
    plt.tight_layout()
    plt.savefig('central_limit_theorem_chi2.png', bbox_inches='tight')
    plt.show()
    
    print("\n考察:")
    print("1. サンプルサイズが小さい場合（n=1, 2, 5）、サンプル平均の分布はカイ二乗分布の形状を保持しており、左右非対称です。")
    ...
    print("5. これは中心極限定理が左右非対称な分布（この場合はカイ二乗分布）に対しても成り立つことを示しています。")
...
def main():
    ...
    # カイ二乗分布の自由度
    df = 3  # 自由度3のカイ二乗分布（平均 = 3, 標準偏差 = sqrt(6) ≈ 2.45）
    ...
    # サンプル平均の分布をプロット
    plot_sample_means_distribution(sample_sizes, num_experiments, df)
